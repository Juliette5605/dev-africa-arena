    ...
    <div class="col-lg-7">
        <div class="admin-card p-4">
            ...
            <form action="{{ route('admin.smtp.update') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <div class="d-flex gap-2 flex-wrap">
                        ...
                        <label class="d-flex align-items-center gap-2 px-3 py-2 rounded-3 fw-bold small"
                               style="cursor:pointer;border:2px solid {{ $config['mailer']===$val?'#f39c12':'#eee' }};background:{{ $config['mailer']===$val?'#fff8eb':'#fff' }};">
                            <input type="radio" name="mailer" value="{{ $val }}"
                                   {{ $config['mailer']===$val?'checked':'' }}
                ...
                {{-- Champs SMTP --}}
                <div id="smtp-fields" style="{{ $config['mailer']==='log'?'display:none':'' }}">
                    <div class="row g-3 mb-3">
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="number" name="port" class="form-control rounded-3 border-0 bg-light py-3"
                                   value="{{ old('port', $config['port']) }}" placeholder="587">
                        </div>
                    </div>
                    <div class="row g-3 mb-3">
                        <div class="col-md-6">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="encryption" class="form-select rounded-3 border-0 bg-light py-3">
                            <option value="tls" {{ $config['encryption']==='tls'?'selected':'' }}>TLS (recommandé pour Gmail — port 587)</option>
                            <option value="ssl" {{ $config['encryption']==='ssl'?'selected':'' }}>SSL (port 465)</option>
                            <option value=""   {{ $config['encryption']===''?'selected':'' }}>Aucun</option>
                        </select>
                    </div>
                </div>
