...
@section('title', $candidature->nom . ' — Voter')
...
<div class="vote-hero">
    <div class="container">
        <div class="back-link">
            <a href="{{ route('vote.leaderboard') }}">← Voir tous les candidats</a>
        </div>
        ...
        <div class="candidat-card">
            ...
            <div class="candidat-header">
                ...
                @if($candidature->photo ?? false)
                    <img src="{{ asset('storage/' . $candidature->photo) }}" alt="{{ $candidature->nom }}" class="candidat-photo">
                @else
                    <div class="candidat-photo-placeholder">
                        {{ strtoupper(substr($candidature->prenom ?? $candidature->nom ?? 'C', 0, 1)) }}
                    </div>
                @endif

                <h1 class="candidat-name">{{ $candidature->prenom ?? '' }} {{ $candidature->nom ?? $candidature->nomcomplet ?? '' }}</h1>
                <p class="candidat-projet">{{ $candidature->projet ?? $candidature->domaine ?? $candidature->expertise ?? '' }}</p>
            </div>

            {{-- Stats --}}
            <div class="stats-row">
                <div class="stat-item">
                    <span class="stat-val">{{ number_format($totalPoints) }}</span>
                    <span class="stat-lab">Points</span>
                </div>
                <div class="stat-item">
                    <span class="stat-val">{{ number_format($totalVotants) }}</span>
                    <span class="stat-lab">Votants</span>
                </div>
                <div class="stat-item">
                    <span class="stat-val">{{ $rang ? '#'.$rang : '—' }}</span>
            ...
            {{-- Formulaire de vote --}}
            <form class="vote-form" method="POST" action="{{ route('vote.store', $voteLink->slug) }}" id="voteForm">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="voter_name" class="form-control" placeholder="Nom complet" required value="{{ old('voter_name') }}">
                    @error('voter_name')<div style="color:#e74c3c;font-size:12px;margin-top:4px">{{ $message }}</div>@enderror
                </div>

                {{-- Téléphone --}}
                <div class="form-group">
                    <label class="form-label">Numéro de téléphone</label>
                    <input type="tel" name="voter_phone" class="form-control" placeholder="+228 XX XX XX XX" required value="{{ old('voter_phone') }}">
                    @error('voter_phone')<div style="color:#e74c3c;font-size:12px;margin-top:4px">{{ $message }}</div>@enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="pricing-grid">
                        @foreach($pricing as $amount => $points)
                        <label class="price-btn" onclick="selectPrice(this)">
                            <input type="radio" name="amount" value="{{ $amount }}" {{ $amount == 500 ? 'checked' : '' }}>
                            <span class="price-amount">{{ number_format($amount) }} F</span>
                            <span class="price-points">{{ $points }} pt{{ $points > 1 ? 's' : '' }}</span>
                        </label>
                        @endforeach
                    </div>
                    @error('amount')<div style="color:#e74c3c;font-size:12px;margin-top:4px">{{ $message }}</div>@enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    @error('payment_method')<div style="color:#e74c3c;font-size:12px;margin-top:4px">{{ $message }}</div>@enderror
                </div>
            ...
            <div class="share-section">
                ...
                <div class="share-buttons">
                    ...
                    @if($voteLink->tiktok_url)
                    <a href="{{ $voteLink->tiktok_url }}" target="_blank" class="share-btn share-tiktok-btn">
                        TikTok
                    </a>
                    @endif
                </div>
