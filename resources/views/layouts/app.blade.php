...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        /* Bouton message en haut */
.chat-bubble {
    position: fixed;
    right: 22px;
    bottom: 136px !important; /* plus haut que btn-dark et btn-top */
    width: 46px;
    height: 46px;
    border-radius: 50%;
    border: none;
    ...
    font-size: 1.05rem;
    background: var(--gold); /* couleur personnalisée */
    color: #fff;
    box-shadow: 0 5px 20px rgba(243,156,18,0.35);
    z-index: 998 !important; /* au-dessus des autres */
}
.chat-bubble:hover {
...
setTimeout(() => {
    ...
    if (f) { f.style.transition = '0.1s'; f.style.opacity = '0'; setTimeout(() => f.remove(), 1); }
}, 1);
...
<div class="modal fade" id="confirmationModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content border-0 rounded-4 shadow-lg text-center p-4">
      ...
      <p class="text-muted" style="color: #6d6b6b !important">{{ session('success') }}</p>
