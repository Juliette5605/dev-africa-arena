...
@section('title', 'Mon Dashboard | DevAfricaArena')
        ...
        <div class="participant-hero mb-3" data-aos="fade-up">
            <div class="d-flex flex-column flex-lg-row justify-content-between gap-4">
                <div>
                    ...
                    @if($candidature)
                        <span class="status-pill" style="background:{{ $currentStatus['bg'] }};color:{{ $currentStatus['color'] }};">
                            <i class="bi bi-stars"></i> {{ $currentStatus['label'] }}
                        </span>
                    @else
        ...
        <div class="row g-3 mb-3">
            <div class="col-md-4" data-aos="fade-up" data-aos-delay="50">
                <div class="dashboard-stat">
                    <div class="dashboard-stat-value">{{ $candidatures->count() }}</div>
                    <div class="dashboard-stat-label">Dossier(s) associe(s)</div>
                </div>
            </div>
            <div class="col-md-4" data-aos="fade-up" data-aos-delay="100">
                <div class="dashboard-stat">
                    <div class="dashboard-stat-value">{{ $candidature?->note ? $candidature->note . '/5' : '--' }}</div>
                    <div class="dashboard-stat-label">Evaluation actuelle</div>
                </div>
            </div>
            <div class="col-md-4" data-aos="fade-up" data-aos-delay="150">
                <div class="dashboard-stat">
                    <div class="dashboard-stat-value">{{ $edition?->nom ?? 'Edition active' }}</div>
                    <div class="dashboard-stat-label">
                        {{ $edition?->date_finale ? 'Finale le ' . $edition->date_finale->format('d/m/Y') : 'Calendrier en cours de mise a jour' }}
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="row g-3">
            <div class="col-lg-7" data-aos="fade-up" data-aos-delay="50">
                <div class="participant-card mb-3">
                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
                        ...
                        @if($candidature)
                            <span class="mini-badge" style="background:rgba(243,156,18,0.1);color:#f39c12;">
                                Depot du {{ $candidature->created_at->format('d/m/Y') }}
                            </span>
                        @endif
                    </div>
                    ...
                    @foreach($steps as $step)
                        <div class="step-row">
                            <div class="step-dot {{ $step['done'] ? 'done' : '' }}"></div>
                            <div>
                                    ...
                                    @foreach($candidatures as $item)
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <div class="text-muted small">{{ $item->expertise }} · {{ $item->niveau }}</div>
                                            </td>
                                            <td>
                                                <span class="mini-badge" style="background:{{ $rowStatus['bg'] }};color:{{ $rowStatus['color'] }};">
                                                    {{ $rowStatus['label'] }}
                                                </span>
                                            </td>
                                        </tr>
            ...
            <div class="col-lg-5">
                ...
                <div class="participant-card mb-3" data-aos="fade-up" data-aos-delay="100">
                    ...
                    @if($candidature)
                        <div class="info-row">
                            ...
                            <span class="fw-bold">{{ $candidature->prenom }} {{ $candidature->nom }}</span>
                        </div>
                        <div class="info-row">
                            <span class="text-muted">Email</span>
                            <span class="fw-bold">{{ $candidature->email }}</span>
                        </div>
                        <div class="info-row">
                            <span class="text-muted">Niveau</span>
                            <span class="fw-bold">{{ $candidature->niveau }}</span>
                        </div>
                        <div class="info-row">
                            <span class="text-muted">Expertise</span>
                            <span class="fw-bold">{{ $candidature->expertise }}</span>
                        </div>
                        <div class="info-row">
                            <span class="text-muted">Pays</span>
                            <span class="fw-bold">{{ $candidature->pays }}</span>
                        </div>
                        <div class="info-row">
                            <span class="text-muted">Diplome</span>
                            <span class="fw-bold">{{ $candidature->diplome }}</span>
                        </div>
                    @else
                        <p class="text-muted mb-0">
                            Aucun dossier n'a ete trouve avec l'email <strong>{{ $user->email }}</strong>.
                        </p>
                    @endif
                </div>
                ...
                <div class="participant-card mb-3" data-aos="fade-up" data-aos-delay="150">
                    ...
                    <div class="step-row">
                        <div class="step-dot {{ $candidature ? 'done' : '' }}"></div>
                        <div>
                    ...
                    <div class="step-row">
                        <div class="step-dot {{ $candidature?->finaliste ? 'done' : '' }}"></div>
                        <div>
                ...
                <div class="participant-card" data-aos="fade-up" data-aos-delay="200">
                    ...
                    <div class="d-flex flex-wrap gap-2">
                        ...
                        <a href="{{ route('criteres') }}" class="btn-outline-gold">
...
const participantAIRoutes = {
    ...
    match: @json(route('dashboard.ai.match'))
};
const participantCsrf = @json(csrf_token());
